function eval = getRAGMergeSegments_vmfmm(combFeat, tmpLabImg, thOptions, parNormal, parColor, rgnb, maskedGT, allInfoSI)
...
for segNum = 1:tNS
    ...
    while(~segmentConsidered(segNum))
        ...
        parNormalT = updateMergedRegionParametersVMFMM(parNormalT, parNormalT, [segNum labelRep], segNum); % Normal
        ...
        for caj = 1:length(adjsToAdd)
            ...
            if(isempty(adjExist))
                ...
                rgnbT.KappaMerged(segNum, tcadj) = mergeKappaVMFMM(parNormalT, [segNum, tcadj]);
...
eval.newLabels = removeSmallObjectsFinally_VMFMM(newLabels, parNormalT, rgnbT);
eval.measure = getEvaluatedResultsRAG2(eval.newLabels, maskedGT);